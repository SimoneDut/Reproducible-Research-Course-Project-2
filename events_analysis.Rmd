...
damage <- rbind(damage[1:9,], c("OTHERS", sum(damage$total_PROPDAMAGE[-1:-9]),
                                sum(damage$total_CROPDAMAGE[-1:-9]), sum(damage$total_DAMAGE[-1:-9]))) %>%
  mutate(total_PROPDAMAGE = as.numeric(total_PROPDAMAGE), total_CROPDAMAGE = as.numeric(total_CROPDAMAGE),
...
ggplot(data = fatalities, aes(x = "", y = total_FATALITIES, fill = EVTYPE)) +
  ...
  theme_void() + theme(plot.title = element_text(hjust = 0.5), plot.caption=element_text(hjust = 0.5, face = "italic")) +
  scale_fill_manual(name = "Event type", values = c(rev(brewer.pal(9, "PuBu")), "#000000")) +
  labs(title = "Fatalities by event type", caption = "From the chart, it is possible to read directly the top 9 events
...
ggplot(data = injuries, aes(x = "", y = total_INJURIES, fill = EVTYPE)) +
  ...
  theme_void() + theme(plot.title = element_text(hjust = 0.5), plot.caption=element_text(hjust = 0.5, face = "italic")) +
  scale_fill_manual(name = "Event type", values = c(rev(brewer.pal(9, "PuBu")), "#000000")) +
  labs(title = "Injuries by event type", caption = "From the chart, it is possible to read directly the top 9 events
...
ggplot(data = damage, aes(x = EVTYPE, y = total/1e9, fill = measure)) + geom_bar(stat="identity") +
  ...
  labs(title = "Damage by event type", caption = "From the chart, it is possible to read directly the top 9 events
